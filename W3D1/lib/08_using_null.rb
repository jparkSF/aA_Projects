...
def null_dept
  ...
  WHERE
    dept_id IS NULL AND teachers.id IS NOT NULL
...
def all_depts_join
  ...
  execute(<<-SQL)
    SELECT DISTINCT teachers.name, dept.name
    FROM depts
    LEFT JOIN
...
def num_teachers_and_mobiles
  ...
  execute(<<-SQL)
  SELECT COUNT(name), COUNT(COALESCE(mobile, '07986 444 2266'))
  FROM teachers
  SQL
end
