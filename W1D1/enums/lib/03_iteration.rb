...
# p [1,2,3,4,5].shuffle.bubble_sort! { |num1, num2| num1 <=> num2 } #=> [1,2,3,4,5]
...
def subwords(word, dictionary)
  ...
  dictionary.each do |word|
    exist_word << word if words.include?(word)
  end
...
class Array
  ...
  def my_select(&prc)
    sel = []
    self.my_each {|el| sel << el if prc.call(el)}
    sel
  end


  def my_inject(acc = self.first, &blk)
    new_arr = []
    blk.call(acc, self)
...
def concatenate(strings)
  strings.my_inject do |concat, str|
    concat + str
  end
end
